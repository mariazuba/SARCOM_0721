...
######################################################################
######################################################################
#source("D:/CTP2017/ANCHOVETA/C?DIGOS/MAE_mar2017/1Pasos_a_Seguir/tama?odeMuestra.R")  

rm(list=ls())   
library(stringr)
source("C:\\MJZ\\CTP2020\\ANCHOVETA\\Salidas\\funciones\\functions.R")
carpetabase<-"C:\\MJZ\\CTP2020\\ANCHOVETA\\"
dir.1<-paste(carpetabase,"codigoinforme\\codigos_admb",sep="")
setwd(dir.1)
#system("admb MAE0919")
system("MAE0720")

#------------------------------------------------
dat.file   = "MAE0720.dat"
data.0        <- lisread(paste(dir.1,dat.file, sep='/'));
names(data.0) <-  str_trim(names(data.0), side="right")
data.1        <- data.0
rep      <- reptoRlist("MAE0720.rep")                                               
std      <- read.table("MAE0720.std",header=T,sep="",na="NA",fill=T) 
